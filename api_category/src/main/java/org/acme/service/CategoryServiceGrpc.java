...
public class CategoryServiceGrpc implements CategoryGrpc {
    ...
    public Uni<Category> createCategory(AddCategory request) {
        ...
        Category.newBuilder().setName(c.getName()).setId(c.id.toString()).build());
    }

    @Override
    public Uni<Category> getCategory(CategoryId request) {
        return CategoryEntity.findById(new ObjectId(request.getId())).onItem().ifNotNull().transform(c->{
            CategoryEntity categoryEntity = (CategoryEntity) c;
            return Category.newBuilder().setId(categoryEntity.id.toString()).setName(categoryEntity.getName()).build();
        });
