...
public class AggregateGrpcService implements AggregateGrpc {
    ...
    @Inject
    public AggregateGrpcService(UniProcess process, ProductMapper productMapper, ProductService productService, PriceService priceService, PlatformService platformService, CategoryService categoryService) {
    ...
    public Uni<Product> createProduct(ProductRequest request) {
        return productService.createProduct(request)
                .onItem().transformToUni(p -> buildProductOnCreate(p, request));
    }

    @Override
    public Uni<Product> findProduct(ProductId request) {
        return productService.findProduct(request.getId())
                .onItem().transformToUni(this::getInformationFromOtherService);
    }
        ...
        return productService.getAllProducts()
                .onItem().transformToUni(productList -> {
                    ...
                    return Uni.createFrom().completionStage(productFuture)
                            .onItem().transform(productMapper::buildList);
                });
    }
    ...
    public Uni<ListOfSearchProduct> searchProduct(Keyword request) {
        return productService.searchProducts(request.getKeyword(), request.getPage())
                .onItem().transformToUni(productList -> {
                    ...
                    return Uni.createFrom().completionStage(productFuture)
                            .onItem().transform(list -> productMapper.buildSearchList(list, (long) productList.size()));
                });
    }

    private Uni<Product> buildProductOnCreate(product.Product p, ProductRequest request) {
        CompletableFuture<List<category.Category>> categoryFuture = process.convertCategoryToFuture(
                categoryService.fetchCategories(p.getCategoryIdList()));
        ...
        CompletableFuture<List<platform.Platform>> platformFuture = process.convertPlatformToFuture(
                platformService.fetchPlatforms(p.getPlatformIdList()));
